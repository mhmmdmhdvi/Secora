...
from lessons.seed_content.reflected_xss import (
    GUIDE,
    GUIDE_TRANSLATIONS,
    LESSON,
    LESSON_TRANSLATIONS,
...
class Command(BaseCommand):
    help = "Seed the Reflected XSS lesson into the relational lesson models."
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(
                "Could not seed Reflected XSS. Check sort_order uniqueness or existing lesson data."
            ) from exc
...
def create_or_replace_draft_revision(lesson, user):
    ...
    return LessonRevision.objects.create(
        ...
        change_summary="Seed from the Reflected XSS interactive lesson.",
        created_by=user,
    )
...
def create_interactive_blocks(section):
    ...
    for locale in LESSON["required_locales"]:
        ...
        LessonBlockTranslation.objects.create(
            ...
            heading="Next: prevent reflected XSS",
...
def create_guide_block(section):
    block = create_block(
        section,
        key="reflected-xss-guide",
        block_type=LessonBlock.Type.SIMULATION,
        sort_order=1,
        config={"registry_key": "reflected-xss-guide"},
    )
    for locale in LESSON["required_locales"]:
