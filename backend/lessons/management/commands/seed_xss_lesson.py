...
from lessons.seed_content.cross_site_scripting import (
    GUIDE,
    GUIDE_TRANSLATIONS,
    LESSON,
    LESSON_TRANSLATIONS,
...
class Command(BaseCommand):
    help = "Seed the Cross-Site Scripting lesson into the relational lesson models."
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(
                "Could not seed Cross-Site Scripting. Check sort_order uniqueness or existing lesson data."
            ) from exc
...
def create_or_replace_draft_revision(lesson, user):
    ...
    return LessonRevision.objects.create(
        ...
        change_summary="Seed from the completed Cross-Site Scripting lesson.",
        created_by=user,
    )
...
def create_guide_block(section):
    block = create_block(
        ...
        block_type=LessonBlock.Type.SIMULATION,
        sort_order=1,
        config={"registry_key": "cross-site-scripting-guide"},
    )
    for locale in LESSON["required_locales"]:
...
def localized_completion_heading(locale):
    if locale == "fa":
        return "بعدی: کاهش خطر XSS"
    return "Next: reduce XSS risk"


def localized_guide_body(locale):
    if locale == "fa":
        return "راهنمای Cross-Site Scripting"
    return "Cross-Site Scripting guide"
