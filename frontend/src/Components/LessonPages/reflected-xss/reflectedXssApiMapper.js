...
export function mapApiLessonToReflectedXssLesson(apiLesson) {
  ...
  const stepBlocks = blocks
    ...
    .sort(bySortOrder);
  const simulation = requiredBlock(blocks, "reflected-xss-demo");
  const completion = requiredBlock(blocks, "completion");
  const guideSection = section(apiLesson, "guide");
  const guide = requiredBlock(guideSection.blocks || [], "reflected-xss-guide");
  const initialState =
  ...
  if (!initialState) {
    throw new Error("Reflected XSS API lesson is missing simulation initial state.");
  }
...
function section(apiLesson, key) {
  ...
  if (!found) {
    throw new Error(`Reflected XSS API lesson is missing ${key} section.`);
  }
  return found;
}
...
function requiredBlock(blocks, key) {
  ...
  if (!block) {
    throw new Error(`Reflected XSS API lesson is missing ${key} block.`);
  }
  return block;
}
...
function richParts(block) {
  ...
  if (!Array.isArray(parts)) {
    throw new Error(`Reflected XSS API block ${block.key} is missing rich parts.`);
  }
  return parts;
}
